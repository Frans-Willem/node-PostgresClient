...
function PostgresTransaction(owner) {
	this.outputQueue=[];
	this.handlerQueue=[];
	this.owner=owner;
}
...
PostgresClient.prototype.startTransaction=function(owner) {
	if (!owner) {
		owner=this;
	} else if (owner.done) {
		throw new Error("Transaction already ended");
	}
	var t=new PostgresTransaction(owner);
	owner.handlerQueue.push(t);
	owner.outputQueue.push(t);
	return t;
};
PostgresClient.prototype.isTransactionFirstOutput=function(tr) {
	var up;
	while (up = tr.owner) {
...
PostgresClient.prototype.endTransaction=function(tr) {
	...
	tr.done=true;
	if (this.isTransactionFirstOutput(tr)) {
		this.flush();
	}
};
PostgresClient.prototype.writeTransaction=function(tr,buffer,handler) {
	...
	if (this.isTransactionFirstOutput(tr) && tr.outputQueue.length===0) {
...
function PostgresCopyInOperation(owner,transaction) {
	EventEmitter.call(this);
...
PostgresCopyInOperation.prototype.write=function(buffer,encoding) {
	...
	this.owner.writeTransaction(this.transaction,PostgresEncoder.create().pushCopyData(Buffer.isBuffer(buffer)?buffer:new Buffer(buffer,encoding)).toBuffer());
	return true;
};
PostgresCopyInOperation.prototype.end=function(buffer,encoding) {
	...
	this.writable=false;
	this.owner.writeTransaction(this.transaction,PostgresEncoder.create().pushCopyDone().toBuffer());
};
PostgresCopyInOperation.prototype.fail=function(msg) {
	...
	this.writable=false;
	this.owner.writeTransaction(this.transaction,PostgresEncoder.create().pushCopyFail(msg,"utf8").toBuffer());
}
	...
	return function(err,type,data) {
		...
		switch (type) {
			...
			case Constants.MessageTypes.Backend.CopyInResponse:
				...
				if (!transactionToEnd || !copyInCallback) {
					//Just throw an error and let the entire thing fail, as doing this without a seperate transaction will mess up the message flow.
					throw new Error("FATAL ERROR: COPY IN operation executed without copyIn callback, message flow is now de-synchronized.");
				}
				if (returned) {
					this.writeTransaction(transactionToEnd,PostgresEncoder.create().pushCopyFail("Command already failed",'utf8').toBuffer());
					return false;
				}
				copyInCallback.call(callback,copyInOperation = new PostgresCopyInOperation(this,transactionToEnd));
				return false;
...
PostgresClient.prototype.simpleQuery=function(query,transaction,callback) {
	...
	if (callback.copyIn) {
		//Create a new (sub-)transaction that will automatically be ended by the query handler once the copy query is done.
		h=createQueryHandler(callback,(transaction=this.startTransaction(transaction)));
	} else {
		//Normal handler
...
PostgresClient.prototype.prepare=function(query) {
	...
	function execute(params,transaction,callback) {
		if (closed) {
		...
		if (preparedName) {
			self.bindExecute(preparedName,params,transaction,callback);
			return;
		}
		preparedCallbacks.push(function(err,name) {
			if (err) {
				...
				self.bindExecute(name,params,transaction,callback);
			}
		});
	}
